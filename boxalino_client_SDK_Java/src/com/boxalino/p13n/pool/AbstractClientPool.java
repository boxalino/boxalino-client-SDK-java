...
public abstract class AbstractClientPool implements ClientPool {
	...
	private ClientWithTHttpClient createClientWithTHttpClient() {
		...
		try {
			...
			return new ClientWithTHttpClient(tHttpClient, client, httpDecorator);
	...
	@Override
	public RuntimeException convertException(Throwable t) {
		if (t instanceof TTransportException || t.getCause() instanceof TTransportException) {
			ensureRefreshed();
			return new RuntimeException("TTransportException exception occurred, refreshing pool", t);
		}
